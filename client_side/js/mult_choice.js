...
function evaluate_answer() {
    ...
    if (sel == corretta) {
        ...
        $('#interazione').append(
            '<button id="prosegui" onclick= "prosegui()"  style="font-size: 3vh;font-family: cursive; border-radius:20px; text-align: center;">Clicca per proseguire</button>',
        );
    } else {
        $('#alert').empty();
        $('#alert').append(
            '<p>Che peccato, hai sbagliato! Ritenta, questa volta non sbaglierai!</p>',
        );
    }
}
...
function prosegui() {
    ...
    if (missionID != 9) window.location.href = `map.html?&id=${myJsonID}`;
    else window.location.href = 'fine.html';
}
