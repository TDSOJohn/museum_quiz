$('body').append(
    '<div class="container-fluid bg-cover" id="sfondo" style="background-color: #54befa; background-repeat: no-repeat;background-position: center; min-height: 100vh; background-size: cover;"><div class="row" style="height: 15vh"><div class="col-sm"><h1 style="text-align: center; font-size:7vh; margin-top:5%; font-family: Brush Script MT"><b><i>Impiccato</i></b></h1></div></div>',
);
$('#sfondo').append(
    '<div class="container"><div class="row" style="height: 15vh"><div class="col-sm"><div id ="risposta" style="text-align: center; font-size:3vh;"></div></div></div></div>',
);
$('#risposta').append(
    '<p id="frase" style=" text-align: center; font-size:5vh; margin-top:10%; font-family: Brush Script MT">La risposta è:</p><div id="interazione"><p id="suggerimento"></p><input type="text" id="insert" style="border-radius:20px; font-size:5vh; font-family: Brush Script MT; text-align: center" onfocus="this.value=\'\'" value="inserisci la risposta"> <br> </input><button style= "border-radius:20px; margin-top:2%; font-family: Brush Script MT"onclick="controlla()">Invia la risposta</button></div>',
);
...
function calcolarandom(vettore) {
    ...
    // cerco indici con "-"
    var betaindex = vettore.map(function (valore, indice) {
        if (valore == 0) {
            return indice;
        }
    });
    ...
    while (
        betaindex.findIndex(function (valore, indice) {
            return valore == undefined;
        }) > -1
    ) {
        // è presente almeno un "undefined" (i valori non zero di bits)
        betaindex.map(function (valore, indice) {
            if (valore == undefined) {
                betaindex.splice(indice, 1);
            }
        });
...
function controlla() {
    ...
    if (inserita.toLowerCase() == parola.toLowerCase()) {
        ...
        $('#interazione').append(
            '<button id="prosegui" onclick= "proseguire()">Clicca per proseguire</button>',
        );
        ...
        bits = rslt[0];
        tentativi = rslt[1]; // Aggiorno variabili
