    ...
    if (method == 'GET') {
        ...
        if (pathName !== '/') {
            ...
            fs.readFile(completePathName, (err, data) => {
                if (err) utilities.errorHandler(err, response);
                else {
            ...
            request
                ...
                .on('end', () => {
                    ...
                    fs.readdir(dbFolder, (err, dbFiles) => {
                        if (err) utilities.errorHandler(err);
                        else {
                            fs.writeFile(
                                `${dbFolder}data${dbFiles.length + 1}.json`,
                                body,
                                'utf8',
                                (err) => {
                            ...
                            QRCode.toFile(
                                qrFilePath,
                                `http://giovanni.basso3.tw.cs.unibo.it/html/gioco.html?id=${
                                    dbFiles.length + 1
                                }`,
                                function (err) {
